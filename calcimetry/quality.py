...
class Quality:
    ...
    def brisque_index(image, max_image_size=89478485):
        x = torch.tensor(np.asarray(image)).permute(2, 0, 1)[None, ...] / 255.0
        gpu_used = False
        ...
        if torch.cuda.is_available() and image_size < max_image_size:
            ...
            x = x.cuda()
            gpu_used = True
        brisque_index: torch.Tensor = piq.brisque(x, data_range=1.0, reduction="none")
        if gpu_used is True:
    ...
    def color_analysis(image, n_clusters):
        ...
        clf = KMeans(n_clusters=n_clusters, n_init="auto")  # top colours
    ...
    def __init__(self, image: Image, n_clusters=3):
        self.image = image
        self.focus = -1
        self.gradient = {"min": -1, "max": -1, "ave": -1, "std": -1}
    ...
    def _compute_colors(self):
        ...
        self.colours = self.color_analysis(
            Quality._prep_image(np.asarray(self.image)), self.n_clusters
        )
